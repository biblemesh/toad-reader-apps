...
const { NOT_LOGGED_IN_MESSAGE } = Constants.expoConfig.extra;
...
const Library = ({
  ...
  const [showEnvironmentUrlsDialog, setShowEnvironmentUrlsDialog] =
    useState(false);
  ...
  const [redirectCheckComplete, setRedirectCheckComplete] = useState(
    !(widget && parent_domain),
  );
  ...
  useEffect(() => {
    if (widget && parent_domain) {
      ...
      safeFetch(
        `${getDataOrigin(Object.values(idps)[0])}/check_for_embed_website_redirect?parent_domain=${encodeURIComponent(parent_domain)}`,
      )
  ...
  useEffect(() => {
    (async () => {
      ...
      try {
        ...
        console.log(
          `/subscriptions GET had an error.`,
          err.message,
          response.status,
        );
      }
  ...
  useEffect(
    ...
    () => {
      ...
      if (
        (pathname === '/' && !['/drawer', '/'].includes(previousPathname)) ||
        ...
        (async () => {
          ...
          for (let accountId in accounts) {
            try {
              ...
              if (
                Object.values(books).some((book) => book.accounts[accountId])
              ) {
  ...
  const ReadIcon = useCallback(
    ({ style }) => (
      <Icon name="menu-book" pack="material" style={[styles.readIcon, style]} />
    ),
    [],
  );
  const ListenIcon = useCallback(
    ({ style }) => (
      <Icon
        name="headphones"
        pack="materialCommunity"
        style={[styles.listenIcon, style]}
      />
    ),
    [],
  );
  ...
  useEffect(() => {
    ...
    if ((books[bookId] || {}).downloadStatus !== 2) {
      if (books[bookId]) {
        Alert.alert(
          i18n('Note'),
          i18n(
            'Download “{{title}}” to view your Reading Schedule for this classroom.',
            '',
            'enhanced',
            books[bookId],
          ),
    ...
    setTimeout(() => {
      ...
      historyPush(`/book/${bookId}`, {
        initialSelectedTabId: 'readingSchedule',
      });
  ...
  useEffect(() => {
    (async () => {
      if (Platform.OS !== 'web' || !logOutAccountId || !redirectCheckComplete)
        return;
      ...
      const usingShibbolethLogin =
        ['SHIBBOLETH'].includes((__DEV__ && devAuthMethod) || authMethod) ||
        doEmailLogin;
  ...
  const validLibraryBookList = library.bookList.filter(
    (bookId) => books[bookId],
  ); // just in case: to prevent error which crashes the app
  let bookList =
    scope == 'all'
      ? validLibraryBookList
      : scope == 'device'
        ? validLibraryBookList.filter(
            (bookId) => books[bookId].downloadStatus == 2,
          )
        : validLibraryBookList.filter((bookId) =>
            Object.keys(books[bookId].accounts).some(
              (accountId) =>
                getIdsFromAccountId(accountId).idpId ===
                getIdsFromAccountId(scope).idpId,
            ),
          );

  if ((library.filter || {}).type === 'metadata') {
    bookList = bookList.filter((bookId) =>
      (books[bookId].metadataValues || []).some(
  ...
  if (useAudiobooks) {
    bookList = bookList.filter(
      (bookId) =>
        !!books[bookId].audiobookInfo === (selectedBookTypeIndex === 1),
    );
  }

  const bookImporterAccountId = Object.keys(accounts).filter(
    (accountId) => accounts[accountId].isAdmin,
  )[0];
  ...
  if (logOutAccountId) {
    ...
    return (
      <SafeLayout>
        ...
        />
        <CoverAndSpin
          text={hasNoAuth ? i18n('Finding books...') : i18n('Logging out...')}
  ...
  if (
    (showLogin || doEmailLogin || query.loginInfo) &&
  ...
  const notLoggedInMessage = showNotLoggedInMessage && (
    ...
    >
      <View style={styles.notLoggedInContainer}>
        <Text style={styles.notLoggedInMessage}>{NOT_LOGGED_IN_MESSAGE}</Text>
  ...
  return (
    <MenuErrorBoundary>
      ...
      >
          ...
          <Switch>
            ...
            {!doingInitialFetch && (
              <Route
                path="/book/:bookId"
                render={() => (
                  <BookWrapper redirectCheckComplete={redirectCheckComplete} />
                )}
              />
            )}
            {!doingInitialFetch && (
            ...
            {!doingInitialFetch && (
              <Route path="/delete-my-account" component={DeleteMyAccount} />
            )}
                  ...
                  {doingInitialFetch ? (
                    ...
                    <View style={styles.content}>
                      {useAudiobooks && bookList.length === 0 && (
                        <Text style={styles.noBooks}>
                          {selectedBookTypeIndex === 0 &&
                            i18n('No e-books found.')}
                          {selectedBookTypeIndex === 1 &&
                            i18n('No audiobooks found.')}
                        </Text>
                      )}
                      ...
                      <LibraryViewer
                        ...
                        viewingAudiobooks={
                          useAudiobooks && selectedBookTypeIndex === 1
                        }
                      />
        ...
        <BookDownloader downloadPaused={/^\/book\//.test(pathname)} />
          ...
          message={
            <>
              <Text style={styles.warningP}>
                {i18n(
                  'Warning: Replacing EPUBs is highly discouraged as it can corrupt user data and enhanced content.',
                  '',
                  'admin',
                )}
              </Text>
              ...
              <Text style={styles.p}>
                {i18n(
                  'For these reasons, replacing an EPUB should only be used as a last resort.',
                  '',
                  'admin',
                )}
              </Text>
              <View style={styles.section}>
                ...
                <Text style={styles.exampleP}>
                  {i18n(
                    'Example 1: If end users do not yet have access to your EPUB, it is often easiest to simply delete it and then import it afresh.',
                    '',
                    'admin',
                  )}
                </Text>
                <Text style={styles.exampleP}>
                  {i18n(
                    'Example 2: If you are wanting to import a new addition of a book, it is often best to do so without deleting the existing version. That way, users with highlights in the existing version will not lose their data while new users can be given access to the new addition.',
                    '',
                    'admin',
                  )}
                </Text>
              ...
              <Text style={styles.p}>
                {i18n(
                  'That said, there are some uncommon scenarios when you may want to replace an existing EPUB.',
                  '',
                  'admin',
                )}
              </Text>
              <View style={styles.section}>
                <Text style={styles.heading}>
                  {i18n(
                    'Valid reasons to consider replacing an EPUB',
                    '',
                    'admin',
                  )}
                </Text>
                ...
                <Text style={styles.exampleP}>
                  {i18n(
                    'Example 2: If you are building out default enhanced content and end users do not yet have access when you discover a need to correct the EPUB, replacing the EPUB is a good option. However, understand that some inline tools may be misplaced or missing as a result.',
                    '',
                    'admin',
                  )}
                </Text>
                <Text style={styles.exampleP}>
                  {i18n(
                    'Example 3: You realize a need to reduce the size of EPUB assets after end users already have access. So long as the assets are not renamed and there are no structure or content changes, user data will not be affected.',
                    '',
                    'admin',
                  )}
                </Text>
              ...
              <Text style={styles.p}>
                {i18n(
                  'If you still wish to replace an EPUB, click confirm below and then upload an EPUB file whose ISBN, Title and Author match those of the existing book to be replaced.',
                  '',
                  'admin',
                )}
              </Text>
        ...
        <Dialog
          ...
          message={['staging', 'beta', 'production'].map((env) => {
            ...
            return (
              <View>
                <View style={styles.p}>
                  <Text style={styles.heading}>{env.toUpperCase()}</Text>
                </View>
                <View style={styles.exampleP}>
                  ...
                  <Text
                    style={styles.a}
                    onPress={() => openURL({ url, historyPush })}
                  >
                    {url}
                  </Text>
                </View>
                <View style={styles.exampleP}>
                  ...
                  <Text
                    style={styles.a}
                    onPress={() => openURL({ url: dataUrl, historyPush })}
                  >
                    {dataUrl}
                  </Text>
                </View>
