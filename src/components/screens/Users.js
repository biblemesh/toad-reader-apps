...
const getIdStr = ({ id, user_id_from_idp, email }) =>
  `[id: ${id}${user_id_from_idp !== email ? `, Tenant id: ${user_id_from_idp}` : ``}]`;

const Users = ({ idps, accounts }) => {
  ...
  const confirmAccountDeletion = useCallback(async () => {
    ...
    try {
      ...
      const response = await safeFetch(
        requestDeletionConfirmationCodeUrl,
        getReqOptionsWithAdditions({
          method: 'POST',
          headers: {
  ...
  const getLoginLink = useCallback(async () => {
    ...
    copy(
      `${window.location.origin}/#/#${JSON.stringify({ doEmailLogin: true, accessCode, back: 0 })}`,
    );
    setCopied(true);
    ...
    async (newSearchStr) => {
      ...
      setSearchResults(
        getSearchResults().filter((item) =>
          getUserSearchLabel(item)
            .toLowerCase()
            .includes(newSearchStr.toLowerCase()),
        ),
      );
      setError();
      setLoading(true);
      ...
      try {
        ...
        if (result.status < 400) {
          ...
          }, 1000 * 30); // delete cache value after a short time
  ...
  return (
    <SafeLayout>
      ...
      <AppHeader
        ...
        titleCentered={true}
        leftControl={
          <HeaderIcon iconName="arrow-back" onPress={historyGoBackToLibrary} />
        }
      />
            ...
            {!!userInfo.id && (
              ...
              >
                ...
                {userInfo.adminLevel !== 'NONE' && (
                  <Text style={styles.userInfoAdminLevel}>
                    {userInfo.adminLevel}
                  </Text>
                )}
                ...
                {!!userInfo.created_at && (
                  <>
                    <Text style={styles.userInfoCreated}>
                      {i18n('Created:', '', 'admin')}
                      {` `}
                      <Text style={styles.userInfoCreatedTime}>
                        {getDateLine({ timestamp: userInfo.created_at })}
                      </Text>
                    </Text>
                    ...
                    <Text style={styles.userInfoCreated}>
                      ...
                      {` `}
                      <Text style={styles.userInfoCreatedTime}>
                        {getDateLine({ timestamp: userInfo.last_login_at })}
                      </Text>
                      {` `}
                      <Text
                        style={styles.userInfoPlatform}
                      >{`(${userInfo.last_login_platform || `Unknown platform`})`}</Text>
                    </Text>
                    ...
                    {!!useEnhancedReader && (
                      <>
                        <Text style={styles.userInfoHeading}>
                          {i18n('Recent Interactive Activity', '', 'enhanced')}
                        </Text>
                          ...
                          ({
                            ...
                            return (
                              <View key={uid} style={styles.userInfoActivity}>
                                  ...
                                  {
                                    {
                                      ...
                                      DISCUSSION_QUESTION: i18n(
                                        'Added a comment to a discussion question',
                                        '',
                                        'enhanced',
                                      ),
                                      SKETCH: i18n(
                                        'Drew on a sketch component',
                                        '',
                                        'enhanced',
                                      ),
                                    }[
                                ...
                                <Text
                                  style={styles.userInfoActivityToolNameLine}
                                >
                                ...
                                <Text style={styles.userInfoActivityBook}>
                                  ...
                                  <Text
                                    style={styles.userInfoActivityBookAuthor}
                                  >
                                    {` `}
                                    {i18n('By:')}
                                    {` `}
                                ...
                                <Text style={styles.userInfoActivityClassroom}>
                                  <Text
                                    style={
                                      styles.userInfoActivityClassroomLabel
                                    }
                                  >
                                  ...
                                  <Text
                                    style={
                                      styles.userInfoActivityClassroomValue
                                    }
                                  >
                                    {classroom_name ||
                                      i18n('[No classroom]', '', 'enhanced')}
                                    {` `}
                                  </Text>
                                </Text>
                                    ...
                                    {
                                      {
                                        ...
                                        QUESTION:
                                          adjustedText.length >
                                          MAX_SNIPPET_LENGTH
                                            ? `...${adjustedText.slice(-MAX_SNIPPET_LENGTH)}`
                                            : adjustedText,
                                        DISCUSSION_QUESTION:
                                          adjustedText.length >
                                          MAX_SNIPPET_LENGTH
                                            ? `${adjustedText.slice(0, MAX_SNIPPET_LENGTH)}...`
                                            : adjustedText,
                                      }[
                                        isDiscussion
                                          ? `DISCUSSION_QUESTION`
                    ...
                    {userInfo.subscriptionInstances.map(
                      (
                        {
                          label,
                          first_given_access_at,
                          expires_at,
                        ...
                        <View key={idx} style={styles.userInfoSub}>
                          <Text style={styles.userInfoSubLabel}>{label}</Text>

                          <Text style={styles.userInfoSubDetail}>
                            {i18n('First given access:', '', 'admin')}
                            {` `}
                            <Text style={styles.userInfoSubDetailTime}>
                              {getDateLine({
                                timestamp: first_given_access_at,
                              })}
                            </Text>
                          </Text>
                          ...
                          {!!expires_at && (
                            <Text style={styles.userInfoSubDetail}>
                              ...
                              {` `}
                              <Text style={styles.userInfoSubDetailTime}>
                                {getDateLine({ timestamp: expires_at })}
                              </Text>
                            </Text>
                          )}
                          ...
                          {!!enhanced_tools_expire_at && (
                            <Text style={styles.userInfoSubDetail}>
                              ...
                              <Text style={styles.userInfoSubDetailTime}>
                                {getDateLine({
                                  timestamp: enhanced_tools_expire_at,
                                })}
                              </Text>
                            </Text>
                          )}
                    ...
                    {userInfo.subscriptionInstances.length === 0 && (
                      <Text style={styles.userInfoNone}>{i18n('None')}</Text>
                    )}
                    ...
                    {userInfo.books.map(
                      ({
                        ...
                        let link = `/#/book/${id}`;
                        try {
                          const { idref: spineIdRef, elementCfi: cfi } =
                            JSON.parse(llCfi);
                        ...
                        return (
                          <View key={id} style={styles.userInfoBook}>
                            <Text style={styles.userInfoBookLine1}>
                              <Text style={styles.userInfoBookTitle}>
                                {title}
                              </Text>
                            ...
                            <Text style={styles.userInfoBookLine2}>
                              ...
                              {(flags || []).length > 0 && (
                                <Text style={styles.userInfoBookDetail}>
                                  ...
                                  {i18n('Flags:')}
                                  {` `}
                                  <Text style={styles.userInfoBookDetailValue}>
                                    {flags.join(', ')}
                                  </Text>
                                </Text>
                              )}
                            ...
                            <Text style={styles.userInfoLinkLine}>
                              <LinkLikeText url={link}>
                                {i18n(
                                  'Go to this user’s latest reading location',
                                  '',
                                  'admin',
                                )}
                              </LinkLikeText>
