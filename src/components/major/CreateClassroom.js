...
const CreateClassroom = React.memo(
  ({
    ...
    const { defaultClassroomUid, sortedClassrooms } = useClassroomInfo({
      books,
      bookId,
      userDataByBookId,
    });
    ...
    const basedOffOptions = useMemo(
      () =>
        sortedClassrooms
          .filter(({ uid, members = [] }) => {
            const myRole =
              (members.filter(({ user_id }) => user_id === userId)[0] || {})
                .role || 'STUDENT';
            return (
              !uid || uid === defaultClassroomUid || myRole === 'INSTRUCTOR'
            );
          })
          .map(({ uid, name }) => ({
            title:
    ...
    const selectedOption = basedOffOptions.filter(
      ({ uid }) => uid === basedOffUid,
    )[0] || { title: '' };
            ...
            <View style={styles.container}>
              ...
              {Platform.OS === 'web' ? (
                ...
                <Select
                  ...
                  value={selectedOption.title}
                  selectedIndex={
                    new IndexPath(basedOffOptions.indexOf(selectedOption))
                  }
                  onSelect={onSelect}
                >
