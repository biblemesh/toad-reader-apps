...
import {
  getIdsFromAccountId,
  getDraftToolByCurrentlyPublishedToolUid,
  splitDraftDataToOptionsAndFrontMatter,
} from '../utils/toolbox';
import useRouterState from './useRouterState';

const useClassroomInfo = ({
  books,
  bookId,
  userDataByBookId = {},
  inEditMode,
  ...
  const book = useMemo(() => books[bookId] || {}, [books[bookId]]);
    ...
    const userClassrooms = (
      ...
      ({ uid, _delete, members = [] }) =>
        ...
        (uid === defaultClassroomUid ||
          members.some(
            ({ user_id, _delete }) => user_id === userId && !_delete,
          )),
    );

    // Ensure the default classroom is always available as an option
    const hasDefaultClassroom = userClassrooms.some(
      ({ uid }) => uid === defaultClassroomUid,
    );
    if (!hasDefaultClassroom) {
      userClassrooms.push({
        uid: defaultClassroomUid,
        name: '', // Will be handled by the UI layer
        members: [],
        tools: [],
  ...
  const [optionsDraftData, frontMatterDraftData] =
    splitDraftDataToOptionsAndFrontMatter((classroom || {}).draftData);
  ...
  const bookVersion =
    !accounts || widget ? 'BASE' : Object.values(accounts)[0].version;
  ...
  const myRole =
    ...
    'STUDENT';
  const iCanEdit =
    (bookVersion === 'PUBLISHER' && isDefaultClassroom) ||
    (myRole === 'INSTRUCTOR' && !isDefaultClassroom);
  ...
  const canViewDashboard =
    !enhancedIsOff && ['INSTRUCTOR', 'ENHANCED'].includes(bookVersion);
  ...
  if (!canViewDashboard && selectedToolUid === 'DASHBOARD') {
    selectedToolUid = null;
  }

  if (selectedToolUid === 'OPTIONS OR SETTINGS' && !canViewOptions) {
    selectedToolUid = null;
  }

  if (!canViewFrontMatter && selectedToolUid === 'FRONT MATTER') {
    selectedToolUid = null;
  }
  ...
  const instructorHighlights = useMemo(
    () =>
      ((classroom || {}).instructorHighlights || []).filter(
        ({ _delete }) => !_delete,
      ),
  ...
  const visibleTools = useMemo(
    ...
    [
      ...
      inEditMode,
      tools,
      draftToolByCurrentlyPublishedToolUid,
      selectedToolUid,
    ],
  );
  ...
  const selectedTool = [
    ...
    'OPTIONS OR SETTINGS',
    'FRONT MATTER',
  ].includes(selectedToolUid)
    ? {}
    : visibleTools.filter(({ uid }) => uid === selectedToolUid)[0];
